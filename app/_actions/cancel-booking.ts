...
export const cancelBooking = actionClient
  ...
  .action(async ({ parsedInput: { bookingId } }) => {
    ...
    // Variável para controlar se houve reembolso
    ...
    if (booking.stripeChargeId) {
      ...
      try {
        ...
        // Marca que o reembolso foi feito com sucesso
        refundIssued = true;
      } catch (error) {
    ...
    // RETORNE UM OBJETO COM A FLAG
    return {
      booking: updatedBooking,
      refundIssued, // true se reembolsou, false se não tinha pagamento ou falhou
    };
  });
